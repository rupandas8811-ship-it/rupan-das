                    ...
                    <div className="space-y-6 sm:space-y-8">
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/5 pb-3">
                                            ...
                                            <div className="flex-shrink-0">
                                                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all ${editingMember.status === MemberStatus.Accepted ? 'bg-green-500/10 border-green-500/20 text-green-500' : editingMember.status === MemberStatus.Deceased ? 'bg-red-500/10 border-red-500/20 text-red-500' : 'bg-orange-500/10 border-orange-500/20 text-orange-400 animate-pulse'}`}>
                                                    {editingMember.status === MemberStatus.Accepted ? <CheckCircle size={10} /> : editingMember.status === MemberStatus.Deceased ? <AlertTriangle size={10} /> : <CheckCircle size={10} className="animate-pulse" />}
                                                    <span className="text-[8px] font-black uppercase tracking-widest whitespace-nowrap">
                                                        {editingMember.status === MemberStatus.Accepted ? 'Verified Identity' : editingMember.status === MemberStatus.Deceased ? 'Deceased' : 'Review Pending'}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5 pb-8">
                            ...
                            <div className="sm:col-span-2 lg:col-span-3">
                                <Input label="Current Address" value={editingMember.address} onChange={(e) => setEditingMember({...editingMember, address: e.target.value})} icon={<MapPin size={12} />} className="py-2.5 text-xs font-bold" />
                            </div>
